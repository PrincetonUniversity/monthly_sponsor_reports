...
class TestSponsorName(unittest.TestCase):
    ...
    def test_sponsor_name(self):
        assert "Pablo G. Debenedetti" == get_full_name_from_ldap(netid="pdebene")

    def test_sponsor_name(self):
        assert None == get_full_name_from_ldap(netid="bigfoot", verbose=False)


class TestSponsor(unittest.TestCase):

    def test_sponsor_per_cluster(self):
        d = {"della":"curt", "stellar":"curt", "tiger":"curt", "tigressdata":"curt", "traverse":"curt", "displayname":"Jonathan D. Halverson"}
        assert get_sponsor_netid_per_cluster_dict_from_ldap(netid="jdh4") == d

        d = {"della":"curt", "stellar":"curt", "tiger":"curt", "tigressdata":"curt", "traverse":"curt", "displayname":"Garrett Wright"}
        assert get_sponsor_netid_per_cluster_dict_from_ldap(netid="gbwright") == d

        d = {"della":None, "stellar":None, "tiger":None, "tigressdata":None, "traverse":None, "displayname":None}
        assert get_sponsor_netid_per_cluster_dict_from_ldap(netid="bigfoot", verbose=False) == d
